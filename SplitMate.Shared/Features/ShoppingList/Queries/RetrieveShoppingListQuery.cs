...
namespace SplitMate.Shared.Features.ShoppingList.Queries
{
	public record RetrieveShoppingListQuery(int ShoppingListId) : IRequest<IResult<RetrieveShoppingListQuery.Response>>
	{
		...
		{
			public record ShoppingListAggregate(int Id, string? Name, decimal SumValue, DateTime CreateDate, string DoneByUserName, int DoneByUserId, bool IsSettled, IReadOnlyList<ShoppingListItem> Items)
			{
